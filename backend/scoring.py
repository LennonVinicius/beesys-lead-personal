...
def commercial_potential(row):
    ...
    if row.get("is_large_chain"):
        score -= 45
        reasons.append("Grande rede: baixa eficiência para prospecção local")
    elif size in {"Pequeno", "Pequeno/médio"}:
        score += 10
        reasons.append("Porte compatível com abordagem local")
    elif size == "Médio/grande":
        score += 2
        reasons.append("Porte maior: pode exigir acesso ao decisor")

    target_fit = _num(row.get("target_fit_score"), int)
    if target_fit is not None:
        score += round((target_fit - 60) * 0.18)
...
def finalize_visit_priority(row):
    ...
    target_fit = float(row.get("target_fit_score") or 75)
    priority = 0.40 * opportunity + 0.27 * potential + 0.14 * probability + 0.19 * target_fit
    status = row.get("pipeline_status") or "NEW"
